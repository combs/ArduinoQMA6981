...
uint8_t QMA6981_writereg(QMA6981_reg_addr addr, uint8_t data);
uint8_t QMA6981_readreg(QMA6981_reg_addr addr, uint8_t len, uint8_t* data);
...
bool QMA6981_set_interrupts(uint8_t interrupt_number, bool step_unsimilar, bool step_quit, bool step, bool d_tap, bool s_tap, bool orient, bool fob, bool fwm, bool ffull, bool data, bool low, bool high_en_z, bool high_en_y, bool high_en_x)
{

    QMA6981_INT_EN0_VAL byte0 =
    {
        .bitmask.STEP_UNSIMILAR_EN = step_unsimilar,
        .bitmask.STEP_QUIT_EN = step_quit,
        .bitmask.STEP_EN = step,
        .bitmask.D_TAP_EN = d_tap,
        .bitmask.S_TAP_EN = s_tap,
        .bitmask.ORIENT_EN = orient,
        .bitmask.FOB_EN = fob
    };
    if(0 != QMA6981_writereg(QMA6981_INT_EN, byte0.val))
    {
        return false;
    }
    QMA6981_INT_EN1_VAL byte1 =
    {
        .bitmask.HIGH_EN_X = high_en_x,
        .bitmask.HIGH_EN_Y = high_en_y,
        .bitmask.HIGH_EN_Z = high_en_z,
        .bitmask.LOW_EN = low,
        .bitmask.DATA_EN = data,
        .bitmask.FFULL_EN = ffull,
        .bitmask.FWM_EN = fwm
    };
    if(0 != QMA6981_writereg(QMA6981_INT_EN + 1, byte1.val))
    {
        return false;
    }

 // not yet mapped to either interrupt pin.
 // to do so with smarts will require storing state.
 // needs to be a class.
